  ...
  <div class="javascript">
    ...
    <br>
     <p>函数的参数只能是按值传递，无论参数是基本类型还是引用类型都只能是值传递。这说明，<b>对于引用类型而言，传入参数不是对象的引用而是引用指向的内存存储的数据，在函数体内操作引用类型的参数会直接修改内存中存储的值，从而影响到传入的实参。</b></p>
      ...
      <p>函数有2种声明方式：函数声明和函数表达式。两者在被解析器解析时，解析器会先读取函数声明，并使其在执行任何代码之前可以被访问，<b>这意味着通过函数声明定义的函数可以被定义在代码中的任何位置。</b>而函数表达式会直到解析器执行到它所在代码行时，它才会被真正解析执行。</p>
      <pre>
      ...
      <p>apply()方法和call()方法：两者的作用是改变被调用函数中this对象的值，两者只是参数写法不同: call(obj,param1,param2,...)</p>
      <b>call(obj,param1,param2,...)</b>
      <b>apply(obj,[param1,param2,...])</b>
      <p>调用方式：<b>funcName().call(thisObj,param1,param2,...)，改变的是funcName()函数中this对象的指向。 </b></p>
      <p>注意点：<b>如果this为null，那么this对象会指向当前环境的全局对象，浏览器中是window，nodejs中是global对象</b> </p>
      ...
      <pre>
        ...
        // 第二行代码 var s2 = s1.substring(2); 相当于在后台实际上执行了一系列的操作
      ...
      <p>总结一下：基本包装类型是特殊的引用类型，它们与普通的引用类型区别在于对象的生存周期：包装类型是由系统自动创建的，它只存在于一行代码执行的瞬间（基本类型调用对应的方法时），随即被销毁，所以不能再运行时为基本类型添加属性和方法；而通过new操作符创建的引用类型实例，在当前作用域会一直存在。</p>
      <p>注意点：
        <b>1.基本类型可以调用方法，但是浏览器后台有做一系列操作让它能够实现（创建一个基本包装类型的对象调用方法后销毁）。</b>
        <b>2.通过new关键字创建的基本包装类型对象也是一个引用类型的对象，所以也可以拥有属性和方法，但是可以调用该包装类型独有的系统内置方法。</b>
        <b>3.不应当通过new关键字创建基本包装类型对象然后附加方法，这样会难以分辨是在处理基本类型还是引用类型。（调用转性函数比如：Boolean()、Number()、String()并不会创建对象，只有使用new关键字时才会创建包装类型的对象）</b>
      </p>
  </div>
</template>
